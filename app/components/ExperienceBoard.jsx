      ...
      <div className="border-[#000000] border rounded-md py-4 px-2 flex flex-row flex-wrap items-center justify-evenly gap-y-4">
        {achievementsList.map((achievement, index) => {
          return (
            ...
            >
              <h2 className="text-slate-700 text-lg sm:text-xl md:text-2xl font-bold flex flex-row items-center">
                {achievement.prefix}
                <AnimatedNumbers
                  includeComma
                  animateToNumber={parseInt(achievement.value)}
                  locale="en-US"
                  className="text-slate-700 text-lg sm:text-xl md:text-2xl font-bold px-0.5"
                  configs={(_, index) => {
                    return {
                      mass: 1,
                      friction: 100,
              ...
              <p className="text-[#45443f] text-xs sm:text-sm md:text-base mt-1">
                {achievement.metric}
              </p>
            </div>
          );
